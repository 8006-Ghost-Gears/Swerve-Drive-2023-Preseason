...
import frc.robot.commands.SetSwerveDriveBalance;
import frc.robot.commands.autos.DriveBackward;
import frc.robot.commands.autos.DriveForward;
//import frc.robot.commands.autos.DriveForward;
import frc.robot.commands.autos.DriveTest;
import frc.robot.commands.autos.YA;
...
public class RobotContainer {
  ...
  //Auto Stabalize Button and LimeLight Button
  JoystickButton Stabalize = new JoystickButton(testController, 5);
  //JoystickButton LimeLightTurn = new JoystickButton(testController, 6);

  //TO-DO Change the limelightturn to the wheels and not the turntable
...
/**
  ...
  public void initializeAutoChooser() {
    ...
    m_autoChooser.addOption(
        "YA", new YA("YA", m_autoBuilder, m_swerveDrive, m_fieldSim));
    SmartDashboard.putData("Auto Selector", m_autoChooser);


    m_autoChooser.addOption(
        "DriveTest", new DriveTest(m_autoBuilder, m_swerveDrive, m_fieldSim));
    SmartDashboard.putData("Auto Selector", m_autoChooser);
    
    //The width and length is done in meters on PathPlanner.
