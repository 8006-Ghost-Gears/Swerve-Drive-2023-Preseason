...
import swervedrivespecialties.swervelib.AbsoluteEncoder;
import swervedrivespecialties.swervelib.Mk4iSwerveModuleHelper;
import swervedrivespecialties.swervelib.SwerveModule;
import frc.robot.Constants;

import frc.robot.lib.subsystems.ClosedLoopSubsystem;
import frc.robot.lib.subsystems.LimelightSubsystem;
import frc.robot.lib.util.Utility;
...
public class SwerveDriveSubsystem extends ClosedLoopSubsystem {
	...
	public SwerveDriveSubsystem(int pigeonId, int[] frontLeftIds, int[] frontRightIds,
			int[] backLeftIds, int[] backRightIds, String shuffleboardTab) {
		ShuffleboardTab tab = Shuffleboard.getTab(shuffleboardTab);

		m_pigeon = new WPI_Pigeon2(pigeonId);
